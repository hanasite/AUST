...
版本 v1.3
变更:
  - 新增「校园 WiFi」卡片：netsh 扫描附近网络 + 一键连接（已保存网络直连，新网络弹窗输密码）
  - 新增命令行自测：python school_auth_v1.3.py --wifi-selftest
  - 沿用 v1.2lite 其余功能：
...
def _decode(raw: bytes) -> str:
    """netsh 输出编码随系统/版本而异：新 Win11 为 UTF-8，传统中文 Windows 为 GBK"""
...
class App(tk.Tk):
    ...
    def _build_wifi_card(self, parent):
        card = self._card(parent, "📶  校园 WiFi")
        card.pack(fill="x", pady=(0, 8))

        top = tk.Frame(card, bg=self.CARD)
        top.pack(fill="x")
        self._wifi_scan_btn = tk.Button(
            top, text="扫描", bg="#E5E7EB", fg="#374151",
            font=self.FONT_S, relief="flat", cursor="hand2",
            padx=10, pady=1,
            command=self._wifi_do_scan
        )
        self._wifi_scan_btn.pack(side="left")
        self._wifi_status_var = tk.StringVar(value="● 未扫描")
        self._wifi_status_lbl = tk.Label(
            top, textvariable=self._wifi_status_var,
    ...
    def _wifi_render(self, nets, current):
        for w in self._wifi_list.winfo_children():
            w.destroy()
        self._wifi_btns = []
        if not nets:
            tk.Label(self._wifi_list, text="（未发现网络，请确认无线网卡已开启）",
                     bg=self.CARD, fg=self.GRAY, font=self.FONT_M,
                     anchor="w", justify="left", wraplength=200).pack(fill="x")
            return
        for n in nets[:self.WIFI_MAX_ROWS]:
            self._wifi_row(n, current)

    def _wifi_row(self, net, current):
        ssid   = net["ssid"]
        target = self.WIFI_TARGET_KEY in ssid.upper()
        done   = bool(current) and ssid == current
        bg     = self.ACCENT_LT if target else self.CARD

        row = tk.Frame(self._wifi_list, bg=bg)
        row.pack(fill="x", pady=1)

        text = ssid if len(ssid) <= 14 else ssid[:13] + "…"
        tk.Label(row, text=text, bg=bg, fg=self.TEXT, anchor="w",
                 font=("Microsoft YaHei", 8, "bold") if target else self.FONT_M
                 ).pack(side="left")
        tk.Label(row, text=f"{net['signal']}%", bg=bg, fg=self.GRAY,
                 font=self.FONT_M).pack(side="left", padx=(5, 0))

        btn = tk.Button(
            row, text="已连接" if done else ("一键连接" if target else "连接"),
            bg=self.SUCCESS if done else (self.ACCENT if target else "#E5E7EB"),
            fg="white" if (done or target) else "#374151",
            activebackground=self.ACCENT_DK, activeforeground="white",
            disabledforeground="white",
            font=self.FONT_T, relief="flat", padx=6, pady=1,
            cursor="arrow" if done else "hand2",
            state="disabled" if done else "normal",
            command=lambda _n=net: self._wifi_do_connect(_n)
        )
        btn.pack(side="right", padx=(5, 0))
        if not done:
            self._wifi_btns.append(btn)

    def _wifi_busy(self, busy: bool, status: str = None):
        self._wifi_scan_btn.config(state="disabled" if busy else "normal")
        for b in self._wifi_btns:
            b.config(state="disabled" if busy else "normal")
        if status is not None:
            self._wifi_status_lbl.config(fg=self.GRAY_DK)
            self._wifi_status_var.set(status)

    def _wifi_do_scan(self):
        self._wifi_scan_btn.config(state="disabled", text="扫描中…")
    ...
    def _wifi_after_scan(self, nets, st):
        self._wifi_scan_btn.config(state="normal", text="扫描")
        self._wifi_last_scan = time.time()
        self._wifi_nets = nets
        cur = st.get("ssid") if st.get("connected") else None
        ts  = time.strftime("%H:%M:%S", time.localtime(self._wifi_last_scan))
        self._wifi_status_var.set(f"{ts} · " + (f"已连接 {cur}" if cur else "未连接"))
        self._wifi_render(nets, cur)
        self._log_msg(f"📶 扫描到 {len(nets)} 个网络" + (f"，当前已连接 {cur}" if cur else ""))

    def _wifi_do_connect(self, net):
        ssid, secured = net["ssid"], net["secured"]
    ...
    def _wifi_after_connect(self, ssid, secured, pwd, ok, msg):
        # 未保存过的加密网络：主线程弹框取密码，再回后台线程连接
        if not ok and msg == "该网络需要密码" and pwd is None:
            pwd = simpledialog.askstring("WiFi 密码", f"「{ssid}」需要密码：",
                                         show="*", parent=self)
            if not pwd:
                self._wifi_busy(False, "已取消")
                return
            self._wifi_busy(True, f"⏳ 连接 {ssid} …")
        ...
        icon = "✅" if ok else ("⏳" if msg.startswith("已发起连接") else "❌")
        self._wifi_busy(False, f"{icon} {msg}")
        self._wifi_status_lbl.config(fg=self.SUCCESS if ok else self.GRAY_DK)
        self._log_msg(f"📶 WiFi {msg}")
        if ok:
            self._wifi_render(self._wifi_nets, ssid)
