...
> 本版本为 v2.0 公测版（Beta），主要用于收集使用意见；欢迎在 Issues 反馈。

说明：学号/WiFi 密码仅保存在本机（已关闭云备份）；WiFi 连接：Android 11+ 通过系统「添加网络」弹窗完成；Android 10 走系统建议机制；Android 9 及以下直接写入系统配置。WiFi 检测/扫描需要：Android 13+ 申请「附近的设备」权限，且所有版本都需要「位置」权限与系统定位开关（国产 ROM 如 vivo/小米 在 13+ 上仍强制要求，这是已知的 Android 生态现实）。
